...
namespace GCM.Controllers
{
    public class StudentController : Controller
    {
        ...
        //[HttpGet]
        //public async Task<IActionResult> DownloadExcelForUpdateStudent(int YearId, int TermId)
        //{

        //    var students = await _userLoginService.GetStudentForUpdate(YearId, TermId);
        ...
        public async Task<IActionResult> DownloadExcelForUpdateStudent(int YearId, int TermId)
        {
            ...
            using (var workbook = new ClosedXML.Excel.XLWorkbook())
            {
                ...
                worksheet.Cell(1, 4).Value = "Enrollment No";
                ...
                foreach (var student in students)
                {
                    ...
                    worksheet.Cell(row, 4).Value = student.Enrollmentid;
                    row++;
                }
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    stream.Position = 0;
                    var fileName = "StudentsData.xlsx";
        ...
        public async Task<IActionResult> Upload(IFormFile excelFile)
        {
            ...
            //    return View(Index); // or however you want to handle errors
            //}

            DataTable studentsDataTable = new DataTable();

            // Define columns according to the expected Excel layout
            studentsDataTable.Columns.Add("Studentid", typeof(long));
            studentsDataTable.Columns.Add("EnrollmentNumber", typeof(string));
            ...
            using (var stream = new MemoryStream())
            {
                ...
                using (var package = new ExcelPackage(stream))
                {
                    ...
                    {
                        string Studentid = worksheet.Cells[row, 1].Text;
                        string enrollmentNumber = worksheet.Cells[row, 4].Text;

                        // Optional: skip blank rows
                        studentsDataTable.Rows.Add(Studentid, enrollmentNumber);
                    }
                }
            }

            // Assuming you have yearId, semid, and userId coming from the form or context

            var regResponse = await _studentService.UpdateEnrollment(studentsDataTable, Convert.ToInt64(userId));
            ...
            {
                TempData["ErrorMessage"] = "Error while updating enrollment.";
            }
